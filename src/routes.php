...
//GET all contacts for debug / Not secure
...
// GET contact with specific id / Secure

    $app->get('/AllContacts', function ($request, $response, $args) {
        $api = verifApi($this->db);

        $contacts = Contacts::displayById($api['idUser']);  
...
// DELETE  contact with related address(es) -> only contact by correct user can be deleted (the user that created the contact) 

    $app->delete('/contact/[{idContact}]', function ($request, $response, $args) {

        $result = verifUser($this->db, $args['idContact']);

        if ($result) {

            $result = Contacts::deleteById($args['idContact']);
        ...
        return $response->withJson($result == true ? ["error" => false, "message" => "entry with id ".$args['idContact']." was deleted"] : ["error" => true, "message" => "entry with id ".$args['id']." was not deleted -> check whether contact specified initially existed or was created by other user or API key correctly specified"], 200, JSON_PRETTY_PRINT);
    });

//POST an address with contact id
...
// GET addresses with specific id contact and user

    $app->get('/addresses/[{idContact}]', function ($request, $response, $args) {

        $api = verifApi($this->db);

        if ($api['idUser']) {

            $address = Addresses::getById($args['idContact'],  intval($api['idUser']));
            return $this->response->withJson($address);

        } else {

            return $this->response->withJson(["error" => true, "message" => "No records found in database"], 200, JSON_PRETTY_PRINT);
        }
    });


// TO DO: DELETE address with specific id contact and user
